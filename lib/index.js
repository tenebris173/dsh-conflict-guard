...
(function () {
  function loadDismissed() {
    try { return localStorage.getItem('dsh-conflict-guard-dismissed') || ''; } catch (e) { return ''; }
  }
  var dismissedSignature = loadDismissed();
  function show(report) {
    if (!report || !document.body) return;
    var hasErrorIssues = (report.issues || []).some(function (i) { return i.severity === 'error'; });
    if ((!report.conflicts || report.conflicts.length === 0) && !hasErrorIssues) return;
      ...
      ids.forEach(function (id) {
        ...
        btn.onclick = function () {
          ...
          fetch('/dsh-conflict-guard/choose', {
            ...
            try { localStorage.setItem('dsh-conflict-guard-dismissed', report.signature); } catch (e) {}
            overlay.remove();
    ...
    (report.issues || []).forEach(function (iss) {
      ...
      var item = document.createElement('div');
      item.style.cssText = 'border:1px solid #e5e7eb;border-radius:10px;padding:10px 12px;margin-bottom:10px;font-size:14px';
      item.textContent = '[错误] ' + iss.message;
      card.appendChild(item);
    });
  ...
  if (typeof ctx.webServer.tapIndex === 'function') {
    ctx.effect(() => ctx.webServer.tapIndex((html) => {
      if (html.includes('dsh-conflict-guard-popup')) return html
      return html.replace(/<\/head>/i, popupScript + '</head>')
    }), 'dsh-conflict-guard: popup script')
  }
  ...
  ctx.effect(() => {
    const watchers = []
    const watchRecursive = (target, handler) => {
      try {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      return await new Promise((resolve) => {
        ...
        const child = isWin
          ? spawn('cmd.exe', ['/c', 'dsh', 'web', '--profile', prof, '--port', '0'], { windowsHide: true })
          : spawn('dsh', ['web', '--profile', prof, '--port', '0'])
        let out = ''
        let done = false
