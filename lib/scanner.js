 ...
 */
function collectRegisterBlocks(code) {
  const blocks = []
...
function collectSlotRegisterBlocks(code) {
  const blocks = []
...
export function extractSlotsFromFile(file) {
  const src = readFileSync(file, 'utf8')
  const code = src.split(/\r?\n/).filter((line) => {
    const t = line.trim()
    return !(t.startsWith('//') || t.startsWith('/*') || t.startsWith('*'))
...
export function scanPackage(packageDir) {
  ...
  if (section && typeof section === 'object') {
    for (const [name, range] of Object.entries(section)) deps[name] = range
  }
