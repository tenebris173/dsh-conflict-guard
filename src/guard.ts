...
import type { Conflict, PluginEntry, RouteScan } from './types.js'
...
/** Compare one candidate plugin against the active roster. */
export function findConflicts(candidate: RouteScan, active: PluginEntry[]): Conflict[] {
...
/** Scan the active roster for pre-existing conflicts (startup / audit mode). */
export function findExistingConflicts(entries: PluginEntry[]): Conflict[] {
  const conflicts: Conflict[] = []
...
export function formatConflicts(conflicts: Conflict[], language: 'zh' | 'en' | 'both' = 'both'): string {
...
export function formatIssues(issues: { severity: 'error' | 'warning'; message: string }[], language: 'zh' | 'en' | 'both' = 'both'): string {
  if (issues.length === 0) {
...
function kindLabelZh(kind: Conflict['kind']): string {
  switch (kind) {
...
function kindLabelEn(kind: Conflict['kind']): string {
  switch (kind) {
